...
public class GraphicsScreen {
	 ...
	 * 
	 * This is synchronised so that multi-threaded access to the list can be safely performed.
	 */
	...
	private class Command {
		...
		void execute(Graphics g) {
			...
			case FILLCIRCLE:
				...
				g.fillOval(penX - radius, penY - radius, radius<<1, radius<<1);	// radius<<1 multiples the radius by 2 (same as radius * 2 but more efficient).
			...
			case RECT:
				...
				g.drawRect(penX, penY, width, height);
			...
			case FILLRECT:
				...
				g.fillRect(penX, penY, width, height);
	...
	public void fillCircle(int radius) {
		...
		// Add the appropriate command to the command list.
		commands.add( new Command(CommandKind.FILLCIRCLE, args) );
	...
	public void Rect(int width, int height) {
		...
		int [] args = {width, height};

		// Add the appropriate command to the command list.
		commands.add( new Command(CommandKind.RECT, args) );

		frame.repaint();
	}
	/**
	 * Draws a solid rectangle at the current pen position using the specified width and height.<br/><br/>
	 * 
	 * The current pen position remains unchanged.
	 * 
	 * @param radius
	...
	public void fillRect(int width, int height) {
		...
		// Add the appropriate command to the command list.
		commands.add( new Command(CommandKind.FILLRECT, args) );
